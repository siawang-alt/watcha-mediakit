...
# --- 탭 선택 ---
tab_home, tab_watcha, tab_pedia, tab_products, tab_faq, tab_contact = st.tabs(
    ["🏠 홈", "🎬 왓챠", "📚 왓챠피디아", "📦 광고 상품", "❓ FAQ", "📩 문의하기"]
)
...
with tab_faq:
    ...
    st.markdown("광고주분들이 자주 문의하시는 내용을 정리했습니다.")
    st.markdown("---")

    with st.expander("광고 집행 최소 기간은 얼마인가요?"):
        st.markdown(
            "모든 광고 상품은 **1주일 단위**로 집행됩니다. "
            "최소 집행 기간은 1주일이며, 연장을 원하시면 추가 구매가 가능합니다."
        )

    with st.expander("광고 소재는 어떤 형식으로 준비해야 하나요?"):
        st.markdown(
            """
대부분의 상품은 **이미지 소재**를 사용하며, 일부 상품(빅배너 등)은 **동영상 소재**도 가능합니다.

- 소재 제작 가이드는 계약 후 별도로 전달드립니다
- 광고 가이드에 따라 소재 수정을 요청드릴 수 있습니다
- 외부 랜딩(클릭 시 외부 사이트로 이동) 가능 여부는 상품마다 다릅니다
"""
        )

    with st.expander("광고 성과는 어떻게 확인하나요?"):
        st.markdown(
            "캠페인 종료 후 **노출수, 클릭수, CTR 등 성과 리포트**를 제공합니다. "
            "집행 중에도 중간 리포트를 요청하실 수 있습니다."
        )

    with st.expander("계약 후 취소하면 위약금이 발생하나요?"):
        st.markdown(
            """
왓챠 광고는 **기간보장 상품**으로, 양 사 날인 이후 변경/취소 시 위약금이 발생합니다.

| 시점 | 위약금 |
|------|--------|
| 광고 집행일로부터 **13~7 영업일 전** | 최종 단가의 **50%** |
| 광고 집행일로부터 **6영업일 전 ~ 라이브 이후** | 최종 단가의 **100%** |
"""
        )

    with st.expander("왓챠와 왓챠피디아에 동시에 광고할 수 있나요?"):
        st.markdown(
            "네, 가능합니다! 두 플랫폼을 묶은 **패키지 상품**도 구성 가능합니다. "
            "문의하기 탭에서 관심 상품을 여러 개 선택해주시면, 최적의 조합을 제안드리겠습니다."
        )

    with st.expander("어떤 업종의 광고를 집행할 수 있나요?"):
        st.markdown(
            "영화, 드라마, 뷰티, 식음료, 가전, 도서 등 **다양한 업종**의 광고 집행이 가능합니다. "
            "단, 일부 상품(랭킹 HOT 배너 등)은 영화/콘텐츠 관련 업종만 가능합니다. "
            "자세한 내용은 문의해주세요."
        )

    with st.expander("광고 집행까지 얼마나 걸리나요?"):
        st.markdown(
            """
일반적인 절차는 다음과 같습니다:

1. **문의 접수** → 1영업일 이내 담당자 연락
2. **미팅/상담** → 상품 및 일정 협의
3. **계약 체결** → 양 사 날인
4. **소재 전달** → 광고 가이드에 맞는 소재 제출
5. **라이브** → 광고 노출 시작

문의부터 라이브까지 보통 **2~3주** 정도 소요됩니다.
"""
        )

    with st.expander("문의는 어떻게 하나요?"):
        st.markdown(
            """
아래 방법 중 편한 방법으로 문의해주세요:

- 이 페이지의 **문의하기** 탭에서 폼 작성
...
"""
    ...
    st.markdown("---")
    st.info("추가 질문이 있으시면 **문의하기** 탭에서 편하게 남겨주세요!")
